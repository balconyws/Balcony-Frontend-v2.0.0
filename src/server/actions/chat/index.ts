...
> => {
  try {
    ...
    const response = await axios.post(`${backendUrl}/api/v2/conversation/start`, payload, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
      setCookies(response.headers['set-cookie']);
      return { data: { conversation: response.data.conversation } };
    }
...
> => {
  try {
    ...
    const backendUrl = process.env.BACKEND_URL;
    const headers = getCookies();
    const response = await axios.get(`${backendUrl}/api/v2/conversation/all`, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
      setCookies(response.headers['set-cookie']);
      return { data: { conversations: response.data.conversations } };
    }
    return { error: { key: '', message: response.data.message } };
  } catch (error: any) {
    if (error.response && error.response.data) {
...
> => {
  try {
    ...
    const response = await axios.put(
      `${backendUrl}/api/v2/conversation/update/${payload.conversationId}`,
...
> => {
  try {
    ...
    const backendUrl = process.env.BACKEND_URL;
    const headers = getCookies();
    const response = await axios.post(`${backendUrl}/api/v2/message/create`, formData, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
      setCookies(response.headers['set-cookie']);
      return { data: { messageId: response.data.messageId, media: response.data.media } };
    }
    return { error: { key: '', message: response.data.message } };
  } catch (error: any) {
    if (error.response && error.response.data) {
...
> => {
  try {
    ...
    const backendUrl = process.env.BACKEND_URL;
    const headers = getCookies();
    const response = await axios.get(`${backendUrl}/api/v2/message/all/${payload.conversationId}`, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
...
export const LastMessageSeen = async (payload: {
  conversationId: string;
  isSeen: boolean;
}): Promise<
  | {
...
> => {
  try {
    ...
    const headers = getCookies();
    const response = await axios.put(
      `${backendUrl}/api/v2/conversation/message-seen/${payload.conversationId}`,
