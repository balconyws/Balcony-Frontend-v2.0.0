...
import { Auto, PaymentType } from '@/types';
...
export const ToggleAutoAcceptBooking = async (): Promise<
  | {
      data: {
        status: Auto;
      };
...
> => {
  try {
    ...
    const response = await axios.get(`${backendUrl}/api/v2/auto/accept-booking`, {
      headers,
...
export const ToggleAutoRentPay = async (payload: {
  tenantId: string;
  type: PaymentType;
}): Promise<
  | {
...
> => {
  try {
    ...
    const backendUrl = process.env.BACKEND_URL;
    const headers = getCookies();
    const response = await axios.post(`${backendUrl}/api/v2/auto/rent-payment`, payload, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
...
> => {
  try {
    ...
    const backendUrl = process.env.BACKEND_URL;
    const headers = getCookies();
    const response = await axios.get(`${backendUrl}/api/v2/auto/status`, {
      headers,
      withCredentials: true,
    });
    if (response && response.data && response.data.success) {
